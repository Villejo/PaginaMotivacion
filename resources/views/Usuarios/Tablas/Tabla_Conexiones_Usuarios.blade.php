...
<div class="panel panel-info">	
	<div class="panel-heading" style="background-color: #562502">
		<h3 class="panel-title">
			...
			<div class="pull-right">
				<strong>Total Conexiones:</strong>
				<label><font size ="3", color color="#000000" face="Tahoma"><strong>{{$Conexion->total()}}</strong></font></label>
			</div>
		</h3>
	</div>
	<div class="panel-body">
			...
			<div class="portlet-body">
				<div class="table-scrollable">
					<table class="table table-striped table-bordered table-advance table-hover">
						...
						<tbody>		
							@foreach ($Conexion as $value)	
							<tr>
								<td class="highlight">										
									EL DIA <strong>{{Carbon::parse($value->fecha_conexion)->toDateString()}}</strong>
									A LAS  <strong>{{Carbon::parse($value->hora_conexion)->format('g:i A')}}</strong>
								</td>	
								<td class="hidden-xs">
									{{$value->ip_conexion}}											
								</td>
								<td class="hidden-xs">	
									{{$value->CodigoUsuario->codigo}}								
								</td>														
							</tr>
							@endforeach	
							<center>{{$Conexion->links()}}</center>
						</tbody>
					</table>
				</div>
			</div>
