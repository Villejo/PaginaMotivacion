...
<input type="hidden" value="{{$Titulo_Mensaje=$value->titulo_mensaje}}">
<div class="col-xs-12 col-sm-12 col col-md-6 col-lg-6">
	<div class="panel panel-success">
		<div class="panel-heading">
			<span class="btn-danger badge btn-md" title="{{$Titulo_Mensaje = strtoupper($Titulo_Mensaje)}}"><b><strong> <font size ="2">{{strtoupper($Titulo_Mensaje)}}
			</font></strong></b></span>
			<strong>({{Carbon::parse($value->hora_notificacion)->diffForHumans()}})</strong>	
			<div class="btn-group pull-right">			
				@if($value->estado=='Si')
				<!-- <input type="checkbox" name="checkbox1" id="checkbox1" checked="true" class="id_mensaje_editar checkbox1"  estado="{{$value->id}}"> -->
				<button class="btn btn-info checkbox_1" estado="{{$value->id}}" estado_mensaje="{{$value->estado}}">
					<i class="fa fa-thumbs-down" aria-hidden="true"></i>
					MENSAJE SIN LEER					
				</button>
				<!-- <input type="checkbox" name="checkbox1" id="checkbox1" class="id_mensaje_editar checkbox1"  estado="{{$value->id}}"> -->
				@else
				<button class="btn btn-warning checkbox_1" estado="{{$value->id}}" estado_mensaje="{{$value->estado}}">
		...
		<div class="panel-body">			
			@if(File::exists($value->imagen_foto))
			<center><img class="cuadradoFoto FotoGrande" src="{{$value->imagen_foto}}" Imagen="{{$value->imagen_foto}}" width="200px" height="200px"/></center>			
			@else
					...
					<div class="row">
						<tbody>
							<tr>
								...
								<td>								
									{{ucfirst($value->Nombre_Equipo->nombre_equipo)}}					
								</td>
							</tr>
							<tr>
								<td>						
							...
							<tr>
								...
								<td>
									{{ucfirst($value->Nombre_Usuario->nombre_usuario)}} {{ucfirst($value->Nombre_Usuario->apellido)}}
								</td>								
							</tr>
							<tr>
								<td>					
								...
								<td>
									{{Carbon::parse($value->hora_notificacion)->toDayDateTimeString()}}
								</td>								
							</tr>
							<tr>
								<td>					
								...
								<td>								
									{{Carbon::parse($value->fecha_notificacion)->toDateString()}} 					
								</td>								
							</tr>
							<td>
							</td>
			...
			<div class="panel-footer">
				<div class="btn-group pull-right">	
					@if(File::exists($value->imagen_foto))
					<a href="{{$value->imagen_foto}}" download="{{$value->imagen_foto}}" title="Descargar Imagen"><strong> <font size ="3", color="#0eacf9" face="Lucida Sans">
						...
						<a href="#" data-toggle = 'modal' data-target="#Modal_Confirmacion_Delete" title="Eliminar Mensaje" class="Eliminar_Notificacion" NombreNotificacion="{{$Titulo_Mensaje = strtoupper($Titulo_Mensaje)}}"  IdNotificacion="{{$value->id}}" <strong> <font size ="3", color="#0eacf9" face="Lucida Sans"><span class="fa fa-trash-o fa-2x"></span></font></a>
					</div>
				</div>
			</div>
		</div>
	...
	@endforeach		
	<center>{{$Notificacion->links()}}</center>		
	@endif
	<!-- Modal See Imagen-->
		...
		$('.CambiarSi').click(function(){
			...
			if(estado_mensaje=='Si'){
				...
				$.ajax({
					...
					url:'{{ url('Cambiar_Estado_Notificacion')}}',
